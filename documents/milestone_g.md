...
| **G4** | **DONE (2026-07-06) — LANDED with released Maia-2.** The per-individual evolving latent adds value over a baseline carrying a **released** SOTA's signal (Maia-2 move-entropy as difficulty), not just a reconstructed proxy. 2 cohorts × 5 seeds: strongest baseline Spearman **0.414 / 0.445 ≥ ChessMimic 0.41**, (B4+z)−B4 = **−0.025 / −0.029, P=1.00**; move-deviation-from-Maia R²≈0.009 (null). Weak-backbone caveat retired for timing. `results/g4_timing.txt`, `scripts/g4_*`. | (baseline+z) − baseline < 0 on held-out timing: **confirmed, both cohorts, every seed.** |
