...
| **G4** | **DONE (2026-07-06) — LANDED with TWO released models.** (a) Maia-2 move-entropy as a difficulty signal: strongest baseline Spearman **0.414/0.445 ≥ ChessMimic 0.41**, (B4+z)−B4 = **−0.025/−0.029, P=1.00**, 2 cohorts × 5 seeds. (b) **AIRTIGHT — Allie (ICLR'25), an actual released think-time head** (Spearman **0.62/0.65**): latent still adds in the direct Allie-vs-Allie+z test both cohorts (**−0.023 P=1.00 / −0.018 P=0.998**); smaller vs Allie, **ns on 2019** against the fullest co-fit (honest). Move-deviation-from-Maia R²≈0.009 (null). Weak-backbone caveat retired for timing vs a released think-time model. `results/g4_timing.txt`, `scripts/g4_{cache,run}_{maia,allie}.py`. | (baseline+z) − baseline < 0 on held-out timing: **confirmed** (Maia every seed; Allie direct test both cohorts, ns in 1/4 co-fit config). |
